...
class View(ttk.Frame, Observer):
    def __init__(self, parent, n_rows, n_columns, square_width):
        ...
        self.board = BoardView(self, n_rows, n_columns, square_width)
...
class BoardView(tk.Canvas):
    def __init__(self, master, n_rows, n_columns, square_width):
        ...
        for i in range(n_rows):
            for j in range(n_columns):
                ...
                x1 = x0 + square_width
                y1 = y0 + square_width
                self._board[i][j] = BoardSquare(self, x0, y0, x1, y1, ipad=ipad)
...
class BoardSquare:
    def __init__(self, canvas, x0, y0, x1, y1, ipad=0):
        self.canvas = canvas
        self.x0 = x0 + ipad
        self.y0 = y0 + ipad
        self.x1 = x1 - ipad
        ...
        self.shape = []
        self.shape_width = (x1 - x0) // 7

    def draw_shape(self, shape):
        self._erase_shape()
        args = (self.canvas, self.x0, self.y0, self.x1, self.y1)
        ids = shape.draw(*args, width=self.shape_width)
